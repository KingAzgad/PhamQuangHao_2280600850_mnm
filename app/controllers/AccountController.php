...
class AccountController
{
    ...
    public function __construct()
    {
        ...
        // Kiểm tra và thêm tài khoản admin mặc định nếu cơ sở dữ liệu chưa có
        try {
            $checkTable = $this->db->query("SHOW TABLES LIKE 'account'");
            if ($checkTable->rowCount() == 0) {
                // Tạo bảng account nếu chưa tồn tại (giả sử bạn đã có SQL tạo bảng)
                $this->db->exec(file_get_contents('path/to/create_account_table.sql'));

                // Thêm tài khoản admin mặc định
                $this->accountModel->save('Admin', 'Administrator', 'Admin@example.com', 'Admin@123', 'admin');
            } else {
                // Kiểm tra xem đã có tài khoản admin chưa
                $admin = $this->accountModel->getAccountByEmail('Admin@example.com');
                if (!$admin) {
                    $this->accountModel->save('Admin', 'Administrator', 'Admin@example.com', 'Admin@123', 'admin');
                }
            }
        } catch (PDOException $e) {
            // Xử lý lỗi nếu cần, nhưng không làm gián đoạn ứng dụng
            error_log("Error initializing admin account: " . $e->getMessage());
        }
    }
