...
class UserController
{
    ...
    public function edit($id)
    {
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            if (!$user) {
                header('Location: /webbanhang/user?error=User not found');
                exit;
            }
    ...
    public function delete($id)
    {
        ...
        if ($user->username === $_SESSION['username']) {
            header('Location: /webbanhang/user?error=Cannot delete yourself');
            exit;
        }

        if ($this->accountModel->deleteUser($id)) {
            header('Location: /webbanhang/user?success=User deleted successfully');
        } else {
            header('Location: /webbanhang/user?error=Failed to delete user');
        }
        exit;
    }
